...
@onready var winter_demand_line: Line2D = $MainFrame/Screen/Demand/DemandWinter
@onready var summer_demand_line: Line2D = $MainFrame/Screen/Demand/DemandSummer
...
func _ready():
	...
	# Set the correct number of points depending on the number of turns
	for i in Gameloop.total_number_of_turns:
		winter_demand_line.add_point(Vector2(0,0))
		summer_demand_line.add_point(Vector2(0,0))
	
	_set_year_x_position()
	_set_line_x_points($MainFrame/Screen/Demand/DemandWinter)
	_set_line_x_points($MainFrame/Screen/Demand/DemandSummer)
	_set_line_y_points($MainFrame/Screen/Demand/DemandWinter, Gameloop.demand_winter, 3000, 10)
	_set_line_y_points($MainFrame/Screen/Demand/DemandSummer, Gameloop.demand_summer, 3000, 5)
	_draw_year_lines()


func _set_year_x_position():
	var distance = $MainFrame/Screen.size.x / Gameloop.total_number_of_turns
	for i in Gameloop.total_number_of_turns:
...
func _draw_year_lines():
	...
	for year in year_x:
		...
		new_line.add_point(Vector2(year, $MainFrame/Screen.size.y), 1)
		$MainFrame/Screen.add_child(new_line)
		
		var new_label = Label.new()
		new_label.text = str(Gameloop.start_year + (i * 3))
		i += 1
		new_label.position = Vector2(new_line.points[1].x - 20, $MainFrame/Screen.size.y + 3)
...
func _set_line_y_points(line, value, scale, increase_rate):
	for i in Gameloop.total_number_of_turns:
		var point = remap(value + (increase_rate * i), 0, scale, $MainFrame/Screen.size.y, 0)
		line.set_point_position(i, Vector2(line.points[i].x, point))
		
#func _add_new_point(line, value, turn, scale):
	#var point = remap(value, 0, scale, $MainFrame/Screen.size.y, 0)
...
#func _change_point(line, new_value, turn, scale, long_term = false):
	#if long_term:
		#for i in Gameloop.total_number_of_turns:
			#var point = remap(new_value, 0, scale, $MainFrame/Screen.size.y, 0)
			#line.set_point_position(i, Vector2(line.points[i].x, point))
	#else:
		#var point = remap(new_value, 0, scale, $MainFrame/Screen.size.y, 0)
