...
func _ready():
	Gameloop.biodiversity_updated.connect(_on_environment_data_updated)
	Gameloop.land_use_updated.connect(_on_environment_data_updated)
...
func _on_environment_data_updated(_percentage):
	...
	var land_use_normalised = Gameloop.land_use * 75 / 100.0
	var biodiversity_normalised = Gameloop.biodiversity * 75 / 100.0
	var tween = get_tree().create_tween()
	tween.tween_property(self, "rotation_degrees",
			clamp(biodiversity_normalised - land_use_normalised, -75, 75), 0.5)
